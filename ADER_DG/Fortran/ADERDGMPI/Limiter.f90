...
!                kk = Face2Neigh(3,iFace) 
           ...
           DO kk = 1, nSubLimV(3)
            DO jj = 1, nSubLimV(2) 
             DO ii = 1, nSubLimV(1)
                 CALL PDESource(src,subuh0(:,ii,jj,kk)+0.5*slopet(:,ii,jj,kk),subpar(:,ii,jj,kk),time+0.5*dt) 
                 subuh1(:,ii,jj,kk) = subuh0(:,ii,jj,kk) - dt/ldx(1)*(Fx(:,ii+1,jj,kk)-Fx(:,ii,jj,kk)) - dt/ldx(2)*(Fy(:,ii,jj+1,kk)-Fy(:,ii,jj,kk)) - dt/ldx(3)*(Fz(:,ii,jj,kk+1)-Fz(:,ii,jj,kk)) + dt*src 
             ENDDO
            ENDDO
           ENDDO 
           !
