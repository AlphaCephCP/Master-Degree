SUBROUTINE InitPointSources(idxe) 
    USE typesDef
    IMPLICIT NONE
    ! Argument list 
    INTEGER :: idxe(IMAX,JMAX,KMAX) 
    ! Local variables 
...
SUBROUTINE RunPointSources 
    ...
    DO i = 1, nPointSource
        ...
        DO iGP = 1, N+1
            ...
            CALL PointSource(sigma,tGP,PointSrc(i)%waveform) 
    ...
    DO i = 1, nPointSource
        ...
        DO kk = 1, nDOF(3) 
         DO jj = 1, nDOF(2) 
          DO ii = 1, nDOF(1) 
            DO iVar = 1, nVar
                duh(iVar,ii,jj,kk,iElem) = duh(iVar,ii,jj,kk,iElem) + PointSrc(i)%phi(ii,jj,kk)*PointSrc(i)%SrcInt(iVar) 
            ENDDO
          ENDDO
         ENDDO  
        ENDDO 
